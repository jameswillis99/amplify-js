...
var initSchema = function (userSchema) {
	...
	Object.keys(schema.namespaces).forEach(function (namespace) {
		var e_1, _a;
		var _b = __read(
				util_1.establishRelationAndKeys(schema.namespaces[namespace]),
				2
			),
			relations = _b[0],
			keys = _b[1];
		...
		Object.values(schema.namespaces[namespace].models).forEach(function (
			model
		) {
			var connectedModels = [];
		...
		while (true && count > 0) {
			...
			try {
				for (
					var _c =
							((e_1 = void 0), __values(Array.from(modelAssociations.keys()))),
						_d = _c.next();
					!_d.done;
					_d = _c.next()
				) {
...
var validateModelFields = function (modelDefinition) {
	return function (k, v) {
		...
		if (fieldDefinition !== undefined) {
			...
			if (
				((!isArray && isRequired_1) || (isArray && !isArrayNullable)) &&
				(v === null || v === undefined)
			) {
			...
			if (types_1.isGraphQLScalarType(type)) {
				...
				var validateScalar_1 =
					types_1.GraphQLScalarType.getValidationFunction(type);
				if (type === 'AWSJSON') {
					...
					if (typeof v === 'string') {
						try {
							...
							throw new Error(
								'Field ' + name_1 + ' is an invalid JSON object. ' + v
							);
						}
					}
				}
				if (isArray) {
					...
					if (
						...
						v.some(function (e) {
							return util_1.isNullOrUndefined(e)
								? isRequired_1
								: typeof e !== jsType_1;
						})
					) {
					...
					if (validateScalar_1 && !util_1.isNullOrUndefined(v)) {
						...
						if (
							!validationStatus.every(function (s) {
								return s;
							})
						) {
							throw new Error(
								'All elements in the ' +
									...
									' array should be of type ' +
									type +
									', validation failed for one or more elements. ' +
									v
							);
						}
					}
...
function getModelConstructorByModelName(namespaceName, modelName) {
	...
	if (isValidModelConstructor(result)) {
		...
		var msg =
			'Model name is not valid for namespace. modelName: ' +
			modelName +
			', namespace: ' +
			namespaceName;
		logger.error(msg);
...
var DataStore = /** @class */ (function () {
	function DataStore() {
				...
				return __generator(this, function (_b) {
					switch (_b.label) {
						...
						case 2:
							...
							return [2 /*return*/];
						case 3:
							this.storage = new storage_1.ExclusiveStorage(
								schema,
								namespaceResolver,
						...
						case 6:
							...
							this.sync = new sync_1.SyncEngine(
								schema,
								namespaceResolver,
								exports.syncClasses,
								userClasses,
							...
							fullSyncIntervalInMilliseconds =
								this.fullSyncInterval * 1000 * 60;
					...
					switch (_a.label) {
						...
						case 1:
							...
							logger.debug('params ready', {
								...
								predicate: predicates_1.ModelPredicateCreator.getPredicates(
									predicate,
									false
								),
								pagination: __assign(__assign({}, pagination), {
									sort: predicates_1.ModelSortPredicateCreator.getPredicates(
					...
					switch (_d.label) {
						...
						case 1:
							...
							if (typeof idOrCriteria === 'string') {
								...
								if (
									!condition ||
									!predicates_1.ModelPredicateCreator.isValidPredicate(
										condition
									)
								) {
									msg =
						...
						case 4:
							(_b = __read.apply(void 0, [_d.sent(), 1])),
								(_c = __read(_b[0], 1)),
								(deleted = _c[0]);
							return [2 /*return*/, deleted];
					}
		...
		this.observeQuery = function (model, criteria, options) {
			return new zen_observable_ts_1.default(function (observer) {
							...
							switch (_a.label) {
								...
								case 1:
									...
									handle = this.observe(
										...
										var isSynced =
											(_c =
												(_b = _this.sync) === null || _b === void 0
													? void 0
													: _b.getModelSyncedStatus(model)) !== null &&
											_c !== void 0
												? _c
												: false;
										var limit =
				...
				var generateSnapshot = function () {
					...
					var isSynced =
						(_b =
							(_a = _this.sync) === null || _a === void 0
								? void 0
								: _a.getModelSyncedStatus(model)) !== null && _b !== void 0
							? _b
							: false;
					var itemsArray = __spread(
				...
				var sortItems = function (itemsToSort) {
					...
					var sortPredicates =
						predicates_1.ModelSortPredicateCreator.getPredicates(
							pagination.sort
						);
				...
				var hubCallback = function (_a) {
					...
					var event = payload.event,
						data = payload.data;
					if (
						event === sync_1.ControlMessage.SYNC_ENGINE_MODEL_SYNCED &&
						((_b = data === null || data === void 0 ? void 0 : data.model) ===
							null || _b === void 0
							? void 0
							: _b.name) === model.name
					) {
		...
		this.configure = function (config) {
			...
			var configDataStore = config.DataStore,
				...
				configPollOffline = config.pollOffline,
			...
			_this.amplifyConfig = __assign(
				__assign({}, configFromAmplify),
				_this.amplifyConfig
			);
			...
			_this.amplifyConfig.authProviders =
				(configDataStore && configDataStore.authProviders) ||
				configAuthProviders;
			_this.pollOffline =
	...
	DataStore.prototype.processPagination = function (
		modelDefinition,
		paginationProducer
	) {
		var sortPredicate;
		...
		if (sort) {
			sortPredicate = predicates_1.ModelSortPredicateCreator.createFromExisting(
				modelDefinition,
				...
				switch (_a.label) {
					case 0:
						...
						return [
							4 /*yield*/,
							Promise.all(
								this.syncExpressions.map(function (syncExpression) {
									return __awaiter(_this, void 0, void 0, function () {
										var _a,
											modelConstructor,
											conditionProducer,
										...
										return __generator(this, function (_b) {
											switch (_b.label) {
												...
												case 1:
													(_a = _b.sent()),
														(modelConstructor = _a.modelConstructor),
														(conditionProducer = _a.conditionProducer);
													modelDefinition =
														getModelDefinition(modelConstructor);
													return [
														4 /*yield*/,
												...
												case 2:
													...
													predicate = this.createFromCondition(
														modelDefinition,
														condition
													);
	...
	DataStore.prototype.createFromCondition = function (
		modelDefinition,
		condition
	) {
		try {
			return predicates_1.ModelPredicateCreator.createFromExisting(
				modelDefinition,
				condition
			);
		} catch (error) {
	...
	DataStore.prototype.retrieveSessionId = function () {
		try {
			...
			if (sessionId) {
				var aws_appsync_graphqlEndpoint =
					this.amplifyConfig.aws_appsync_graphqlEndpoint;
