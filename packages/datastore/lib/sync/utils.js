...
function getNonModelFields(namespace, modelDefinition) {
	...
	Object.values(modelDefinition.fields).forEach(function (_a) {
		...
		if (types_1.isNonModelFieldType(type)) {
			...
			var scalarFields = Object.values(getScalarFields(typeDefinition)).map(
				function (_a) {
					var name = _a.name;
					return name;
				}
			...
			Object.values(typeDefinition.fields).forEach(function (field) {
				...
				if (types_1.isNonModelFieldType(type)) {
					...
					nested_1.push(
						name +
							' { ' +
							generateSelectionSet(namespace, typeDefinition_1) +
							' }'
					);
				}
			});
			result.push(
				name + ' { ' + scalarFields.join(' ') + ' ' + nested_1.join(' ') + ' }'
			);
		}
	});
	return result;
...
function buildGraphQLOperation(namespace, modelDefinition, graphQLOpType) {
	...
	switch (graphQLOpType) {
		case 'LIST':
			...
			documentArgs =
				'($limit: Int, $nextToken: String, $lastSync: AWSTimestamp, $filter: Model' +
				typeName +
				'FilterInput)';
			operationArgs =
...
exports.createMutationInstanceFromModelOperation =
		...
		return __generator(this, function (_b) {
			switch (_b.label) {
				...
				case 3:
					error_1 = _b.sent();
					logger.debug(
						'Error getting auth modes for model: ' + modelName,
						error_1
					);
					return [3 /*break*/, 4];
				case 4:
...
function getTokenForCustomAuth(authMode, amplifyConfig) {
	...
	return __awaiter(this, void 0, void 0, function () {
		...
		return __generator(this, function (_b) {
			switch (_b.label) {
				case 0:
					if (!(authMode === api_graphql_1.GRAPHQL_AUTH_MODE.AWS_LAMBDA))
						return [3 /*break*/, 6];
					(_a = amplifyConfig.authProviders),
						(functionAuthProvider = (
							_a === void 0 ? { functionAuthProvider: null } : _a
						).functionAuthProvider);
					if (
						!(
							functionAuthProvider && typeof functionAuthProvider === 'function'
						)
					)
						return [3 /*break*/, 5];
					_b.label = 1;
				...
				case 3:
					error_2 = _b.sent();
					throw new Error(
						'Error retrieving token from `functionAuthProvider`: ' + error_2
					);
				case 4:
					return [3 /*break*/, 6];
				case 5:
					...
					throw new Error(
						'You must provide a `functionAuthProvider` function to `DataStore.configure` when using ' +
							api_graphql_1.GRAPHQL_AUTH_MODE.AWS_LAMBDA
					);
				case 6:
					return [2 /*return*/];
			}
