...
var SyncProcessor = /** @class */ (function () {
		...
		Object.values(this.schema.namespaces).forEach(function (namespace) {
			Object.values(namespace.models)
				...
				.forEach(function (model) {
					var _a = __read(
							utils_1.buildGraphQLOperation(namespace, model, 'LIST'),
							1
						),
						_b = __read(_a[0]),
	...
	SyncProcessor.prototype.graphqlFilterFromPredicate = function (model) {
		...
		var predicatesGroup = predicates_1.ModelPredicateCreator.getPredicates(
	...
	SyncProcessor.prototype.retrievePage = function (
		...
		return __awaiter(this, void 0, void 0, function () {
			...
			return __generator(this, function (_d) {
				switch (_d.label) {
					case 0:
						(_a = __read(this.typeQuery.get(modelDefinition), 2)),
							(opName = _a[0]),
							(query = _a[1]);
						variables = {
							limit: limit,
									...
									switch (_b.label) {
										...
										case 2:
											...
											if (authModeAttempts >= readAuthModes.length) {
												...
												logger.debug(
													'Sync failed with authMode: ' + authMode,
													error_1
												);
												if (
													utils_1.getClientSideAuthError(error_1) ||
													utils_1.getForbiddenError(error_1)
												) {
													// return empty list of data so DataStore will continue to sync other models
													logger.warn(
														'User is unauthorized to query ' +
															opName +
															' with auth mode ' +
															authMode +
										...
										return __generator(this, function (_a) {
											switch (_a.label) {
												...
												case 3:
													error_2 = _a.sent();
													clientOrForbiddenErrorMessage =
														utils_1.getClientSideAuthError(error_2) ||
														utils_1.getForbiddenError(error_2);
													if (clientOrForbiddenErrorMessage) {
														throw new core_1.NonRetryableError(
															clientOrForbiddenErrorMessage
														);
													}
													hasItems = Boolean(
													...
													if (this.partialDataFeatureFlagEnabled()) {
														if (hasItems) {
															result = error_2;
															result.data[opName].items = result.data[
																opName
													...
													unauthorized =
														error_2 &&
														error_2.errors &&
														error_2.errors.some(function (err) {
													...
													if (unauthorized) {
														...
														if (hasItems) {
															...
															result.data[opName] = __assign(
																__assign({}, opResultDefaults),
																result.data[opName]
															);
														}
														logger.warn(
